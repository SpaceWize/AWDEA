      ...
      <div className="mx-auto max-w-4xl">
        ...
        <StaggerContainer className="flex flex-col gap-4 md:hidden">
          {donors.map((donor) => (
            ...
            >
              <p className="mb-1 text-lg font-bold text-slate-900">{donor.name}</p>
          ...
          <div className="overflow-x-auto rounded-2xl border border-slate-200">
            <table className="w-full border-collapse text-left">
              ...
              <thead className="bg-[var(--color-mist)]">
                <tr>
                  <th scope="col" className="px-6 py-4 text-xs font-bold uppercase tracking-[0.14em] text-slate-600">
                    Name
                  </th>
                  <th scope="col" className="px-6 py-4 text-xs font-bold uppercase tracking-[0.14em] text-slate-600">
                    Date
              ...
              <tbody>
                {donors.map((donor) => (
                  <tr
                    key={`${donor.name}-${donor.date}`}
                    className="border-t border-slate-200 transition-colors duration-500 hover:bg-[var(--color-mist)]"
                  >
                    <th scope="row" className="px-6 py-5 font-semibold text-slate-900">
                      {donor.name}
                    </th>
                    <td className="px-6 py-5 text-slate-600">{donor.date}</td>
